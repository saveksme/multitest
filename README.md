...
Шрифт — **Manrope** (геометричный, в духе Google Sans, с кириллицей); скачивается автоматически,
запас — Roboto/Noto/DejaVu. Рендер: `rsvg-convert` (или ImageMagick) — ставится автоматически.

**Хостинг картинки.** По умолчанию заливается на catbox. Но catbox отклоняет **анонимную**
загрузку файлов с хостинг/VPS-адресов (HTTP 412 «Invalid uploader»), поэтому без аккаунта
сводка уходит на **litterbox** (litter.catbox.moe — то же семейство, но ссылка живёт до 72 ч),
а если и он недоступен — на запасные хостинги. Для **постоянных** ссылок именно на catbox
укажите userhash аккаунта:

```bash
CATBOX_USERHASH=<ваш_хэш> multitest
```
